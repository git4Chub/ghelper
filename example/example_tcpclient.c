...
* APN Param
******************************************************************/
static u8 m_apn[MAX_GPRS_APN_LEN] = "uninet";
...
static u8  m_SrvADDR[SRVADDR_BUFFER_LEN] = "119.23.150.112";
static u32 m_SrvPort = 5001;
    ...
    else if (TCP_TIMER_ID == timerId)
    {
        ...
        switch (m_tcp_state)
        {
            ...
            case STATE_SOC_SEND:
            {
                ...
                do
                {
                    ret = Ql_SOC_Send(m_socketid, m_RxBuf_Uart, sizeof(m_RxBuf_Uart));
